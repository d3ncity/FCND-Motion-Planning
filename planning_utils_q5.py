...
def create_graph_with_edges(edges):
    G = nx.Graph()
    for e in edges:
        G.add_edge(e[0],e[1], weight=heuristic(e[0], e[1]))
        print("An edge was added, P1 and P2 are {0} and {1}".format(e[0], e[1]))
    return G

#Question 5 - Breadth First Search on the Graph
def bfs(graph, start, goal):

    q1 = queue.Queue() # queue for bfs
    q1.put(start)
    print('\nThe Graph data\n{}'.format(nx.classes.function.info(graph,n=None)))
    path = []
    path_cost = 0
    visited = set(start)
    ...
    while not q1.empty():
        ...
        if item == goal:        
            ...
            for next_node in graph.neighbors(item):
                branch_cost = current_cost + 1 #just add 1 for the cost to perform the action
